...
export interface Competition {
  ...
  counts: {
    listed: number;
    evidenced: number;
    partners_who_also_compete: number;
    total?: number;
  };
...
export interface DiagramNode {
  label: string;
  provider: "orange" | "partner" | "customer" | "third_party";
}
...
export interface Topic {
  ...
  market_size_summary?: {
    method: string;
    sam_base: number | null;
    tam_base: number | null;
    confidence: string;
  } | null;
...
export interface PlannerMeta {
  ...
  workflow_stages: { id: string; label: string }[];
}
