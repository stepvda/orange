    ...
    >
      ...
      <label>
        ...
        >
          ...
          <option value="strategic_coverage">
            Maximise strategic coverage
          </option>
        </select>
        <em>
          Different objectives give materially different portfolios. This is a
          decision.
        </em>
      </label>
      ...
      <label>
        ...
        <input
          ...
          min={1}
          max={60}
          value={req.entry_slots_per_year ?? 12}
      ...
      <label>
        ...
        />
        <em>
          {Math.round((req.pool_availability ?? 0.15) * 100)}% of each pool. The
          rest runs the existing business.
        </em>
      </label>
      ...
      <label>
        ...
        <select
          value={req.max_portfolio_distance}
          onChange={(e) =>
            set("max_portfolio_distance", Number(e.target.value))
          }
        >
      ...
      <label>
        ...
        <input
          ...
          onChange={(e) =>
            set("max_share_per_vertical", Number(e.target.value))
          }
        />
        <em>
          {Math.round((req.max_share_per_vertical ?? 0.4) * 100)}%. Without a
          cap the optimiser returns a single-vertical plan and calls it a
          portfolio.
        </em>
      </label>

      <label>
        <span>Furthest workflow stage</span>
        <select
          value={req.workflow_stages?.[0] ?? ""}
          onChange={(e) => {
            const ids = meta.workflow_stages?.map((s) => s.id) ?? [];
            const idx = ids.indexOf(e.target.value);
            set("workflow_stages", idx >= 0 ? ids.slice(idx) : []);
          }}
        >
          <option value="">Any stage — unfiltered</option>
          {meta.workflow_stages?.map((s) => (
        ...
        <em>
          Restricts the plan to spaces already at or past this stage in the gate
          (§4.10).
        </em>
      </label>
      ...
      <p className="pl-form-note">
        Selection and projection are arithmetic — no model call, so this is
        fast. The written business plan is a separate step.
      </p>
    </form>
  );
}
...
export default function PlannerScreen({
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape" && !document.querySelector(".help-backdrop"))
        onClose();
    };
  ...
  const narrate = useCallback(async () => {
    ...
    setError(null);
    try {
      setPlan(await api.narratePlan(plan.id));
      setTab("narrative");
    } catch (e: any) {
      setError(String(e.message ?? e));
    } finally {
  ...
  const exportPdf = useCallback(async () => {
    ...
    setError(null);
    try {
      setReport(await api.buildPlanReport(plan.id));
      setTab("document");
    } catch (e: any) {
      setError(String(e.message ?? e));
    } finally {
    ...
    <div className="fs-screen pl-screen">
        ...
        <div className="fs-title">
          <div className="fs-badges">
            ...
            {meta && (
              <span
                className="badge"
                title="The assumption set this plan rests on"
              >
          ...
          <p className="pl-provenance-head">
            ...
            {meta?.filed
              ? ` · segment EBITDAaL margin ${(meta.filed.segment_ebitdaal_margin * 100).toFixed(1)}%` +
        ...
        {plan && (
          <div className="fs-panes" role="group" aria-label="View">
            {TABS.map((t) => (
              ...
              >
                {TAB_LABELS[t]}
                {t === "narrative" && !plan.narrative && (
                  <span className="fs-stale-dot">●</span>
                )}
              </button>
            ))}
          </div>
      ...
      <div className="fs-body pl-body">
        <aside className="pl-side">
          <h3>Parameters</h3>
          {meta ? (
            <Form meta={meta} busy={busy} onRun={run} />
          ) : (
        ...
        <main className="pl-main">
          ...
          {!plan && !busy && (
            <div className="pl-empty">
              ...
              <p className="pl-filings">
                <strong>Grounded in Orange&apos;s filed accounts.</strong> The
                margin applied to revenue and the rate used to discount it are
                quoted from{" "}
                {meta?.source_filing ??
                  "Orange's Universal Registration Document"}{" "}
                — not chosen here. Every projection is also checked against the
                segment revenue Orange filed, so a plan that implies implausible
                growth says so.
              </p>
              <p>
                Set the constraints on the left and build one. The optimiser
                selects a set of opportunity spaces, schedules when each is
                entered, and projects revenue and profit across the plan window.
              </p>
              <p className="ca-muted">
                It is a scenario under stated assumptions, not a forecast. Every
                projection carries its interval and is checked against Orange's
                own filed segment revenue.
              </p>
            </div>
          )}
          ...
          {plan && p && tab === "overview" && (
            <>
              <div className="pl-stats">
                ...
                <Stat
                  label="5-year profit"
                  value={EUR(p.profit_total)}
                  tone="accent"
                  sub={`band ${EUR(p.profit_total_low)} – ${EUR(p.profit_total_high)}`}
                />
                <Stat
                  label={`NPV @ ${(p.discount_rate * 100).toFixed(1)}%`}
                  value={EUR(p.npv_profit)}
                ...
                />
                <Stat
                  label="Year-5 vs segment"
                  value={`${((p.year5_share_of_segment ?? 0) * 100).toFixed(1)}%`}
                  tone={
                    (p.year5_share_of_segment ?? 0) > 0.15 ? "warn" : undefined
                  }
              ...
              {(plan.capacity_usage?.binding ?? []).length > 0 && (
                <div className="pl-binding">
                  ...
                  <p className="ca-muted">
                    These are the constraints the optimiser hit. Relaxing one is
                    what would change the answer.
                  </p>
                  <ul>
                    {plan.capacity_usage!.binding!.map((b) => (
                      <li key={b}>{b}</li>
                    ))}
                  </ul>
                </div>
            ...
            <>
              ...
              <table className="data pl-table">
                <thead>
                  <tr>
                    ...
                    <th>L</th>
                    <th>Pool</th>
                    <th className="num">5y revenue</th>
                    <th className="num">5y profit</th>
                  </tr>
                </thead>
                <tbody>
                  {plan.selections.map((s) => (
                    <tr key={s.opportunity_id}>
                      <td>
                        <span className="badge">Y{s.entry_year}</span>
                      </td>
                      <td>
                        <button
                          className="pl-link"
                          onClick={() => onOpenTopic?.(s.opportunity_id)}
                        >
                          {s.opportunity_id}
                        </button>
                      ...
                      <td className="num">
                        {EUR(s.revenue_by_year.reduce((a, b) => a + b, 0))}
                      </td>
                      <td className="num">
                        {EUR(s.profit_by_year.reduce((a, b) => a + b, 0))}
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              {(plan.exclusions ?? []).length > 0 && (
                <details className="pl-exclusions">
                  <summary>
                    {plan.exclusions.length} near-miss space(s), and why each
                    was left out
                  </summary>
            ...
            <div className="pl-narrative">
              {!plan.narrative && (
                <div className="pl-empty">
                  ...
                  <p>
                    Everything on the Overview tab was computed. This step asks
                    a model to explain it — the thesis, the sequence, the
                    execution story, the risks and what was deliberately left
                    out.
                  </p>
                  <p className="ca-muted">
                    It may not introduce a number. Every figure already sits in
                    the projection, and a sentence that disagreed with it would
                    be a defect the reader has to adjudicate — so a section
                    containing a quantity is stripped and shown as such.
                  </p>
                  <button
                    className="pl-run"
                    disabled={writing}
              ...
              {plan.narrative && (
                <>
                  <blockquote className="pl-headline">
                  ...
                  {(plan.stripped ?? []).length > 0 && (
                    <details className="ca-stripped">
                      <summary>
                        {plan.stripped.length} section(s) removed by the
                        guardrails
                      </summary>
                      <ul>
                        {plan.stripped.map((s, i) => (
                          <li key={i}>
                            <strong>{s.section}</strong> — {s.reason}
                          </li>
                        ))}
                      </ul>
                    </details>
          ...
          {plan && tab === "assumptions" && (
            <div className="pl-assumptions">
              ...
              <p className="ca-muted">
                Two figures are quoted from Orange's own filed accounts.
                Everything else is a planning band with an owner — and a plan
                built under one version of these assumptions is not comparable
                with a plan built under another.
              </p>
              <h4>Filed — {plan.assumptions?.source_filing}</h4>
              <table className="data">
                <tbody>
                  <tr>
                    <td>Discount rate (post-tax)</td>
                    <td className="num">
                      {(
                        (plan.assumptions?.filed?.discount_rate_post_tax ?? 0) *
                        100
                      ).toFixed(1)}
                      %
                    </td>
                  ...
                  <tr>
                    <td>Segment EBITDAaL margin</td>
                    <td className="num">
                      {(
                        (plan.assumptions?.filed?.segment_ebitdaal_margin ??
                          0) * 100
                      ).toFixed(1)}
                      %
                    </td>
                  ...
                  <tr>
                    ...
                    <td className="num">
                      €
                      {(
                        plan.assumptions?.filed?.segment_revenue_eur_m ?? 0
                      ).toLocaleString()}
                      m
                    </td>
                  </tr>
              ...
              <table className="data">
                <tbody>
                  {Object.entries(
                    plan.assumptions?.margin_by_distance ?? {},
                  ).map(([k, v]) => (
                    <tr key={k}>
                      <td>Margin at {k}</td>
                      <td className="num">
                  ...
                  {Object.entries(plan.assumptions?.ramp_by_horizon ?? {}).map(
                    ([k, v]) => (
                      <tr key={k}>
                        <td>Ramp — {k}</td>
                        <td className="num">
              ...
              <p className="pl-provenance">
                economics {plan.economics_version} · sizing{" "}
                {plan.sizing_version} · weights {plan.weight_set}
            ...
            <div className="pl-doc">
              <div className="pl-doc-bar">
                <div>
                  ...
                  Inputs, projection, every selected space with its own
                  description, the business plan and the assumptions it rests
                  on.
                  {!plan.narrative && (
                    <span className="pl-doc-warn">
                      {" "}
                      The business plan has not been written yet — generate it
                      first and the document will include it.
                    </span>
                  )}
                </div>
                <span className="spacer" />
                {report && (
                  <>
                    <a
                      className="pl-doc-btn"
                      href={api.planReportUrl(plan.id, report.content_hash)}
                      target="_blank"
                      rel="noreferrer"
                    >
                      Open in a new tab
                    ...
                    <a
                      className="pl-doc-btn"
                      href={api.planReportDownloadUrl(plan.id)}
              ...
              {report && (
                <>
                  <object
                    className="pl-doc-frame"
                    data={api.planReportUrl(plan.id, report.content_hash)}
                  ...
                  >
                    ...
                    <p>
                      ...
                      <a href={api.planReportDownloadUrl(plan.id)}>
