...
serves the built React bundle from the same origin. 64 endpoints.
...
| `GET` | `/api/topics/{topic_id}/presales` | The whole pre-sales catalogue, each entry with its build state per format. |
| `POST` | `/api/topics/{topic_id}/presales/{kind}` | Build one piece. `?fmt=` picks the format, `?force=true` rebuilds a current one. |
| `GET` | `/api/topics/{topic_id}/presales/{kind}/file` | The file. `?fmt=` selects it, `?download=1` sends it as an attachment. |
